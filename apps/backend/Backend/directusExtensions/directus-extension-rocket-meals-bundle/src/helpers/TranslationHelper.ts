...
export type TranslationUpdateContext<E extends ExistingTranslation> = {
...
export class TranslationHelper {
  ...
  >(
    ...
    context: TranslationUpdateContext<E>
  ) {
    const { translationsFromParsing, items_primary_field_in_translation_table, itemsTablename, myDatabaseHelper } = context;
  ...
  >(
    ...
    const { itemsTablename, myDatabaseHelper } = context;
    const specificItemServiceReader = await myDatabaseHelper.getItemsServiceHelper<T>(itemsTablename);
    ...
    return TranslationHelper.updateItemTranslationsForItemWithTranslationsFetched(itemWithTranslations, context);
  }
