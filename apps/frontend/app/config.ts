...
// getCustomer.ts is generated by scripts/generateCustomer.sh in CI and returns the
// customer name (swosy / studi-futter / test) baked directly into the source tree.
// This is the single source of truth for the customer – it replaces relying on
// env variables which are not reliably propagated into the app bundle.
export function getCustomerEnvVariable(): string | undefined {
	try {
		// eslint-disable-next-line @typescript-eslint/no-var-requires
		const mod = require('./getCustomer');
		return typeof mod.getCustomer === 'function' ? mod.getCustomer() : undefined;
	} catch {
		return undefined;
	}
}
