...
const HourSheet: React.FC<HourSheetProps> = ({ closeSheet }) => {
	...
	const getSortedWeekdayKeys = () => {
		...
		const firstDayOfTheWeekKey = firstDayOfTheWeek?.id || 'monday';
	...
	const fetchSelectedBuilding = async () => {
		...
		try {
			...
			const sortedDayKeys = getSortedWeekdayKeys();
			...
			Object.entries(groupedByGroupName).forEach(([groupId, groupData]) => {
				...
				hoursList.forEach((entry: any) => {
					sortedDayKeys.forEach(day => {
						if (entry[day]) {
							const timesForDay = dayStartAndEndTimeDict[day] || [];

							const time_start_as_string = entry.time_start; // "08:00:00"
							const time_end_as_string = entry.time_end; // "20:00:00"
				...
				for (let i = 0; i < sortedDayKeys.length; i++) {
					const currentDay = sortedDayKeys[i];
					const currentTimeRanges = dayConsecutiveRanges[currentDay];
					if (!currentTimeRanges || currentTimeRanges.length === 0) {
						...
						const isLastDay = i === sortedDayKeys.length - 1;
				...
				groupedTimes.forEach(timeRange => {
					...
					const time_start_in_seconds = parseInt(timeRange.time_start);
					const time_end_in_seconds = parseInt(timeRange.time_end);
					const time_start_hours = Math.floor(time_start_in_seconds / 3600);
					const time_start_minutes = Math.floor((time_start_in_seconds % 3600) / 60);
					const time_start_seconds = time_start_in_seconds % 60;
					const time_end_hours = Math.floor(time_end_in_seconds / 3600);
					const time_end_minutes = Math.floor((time_end_in_seconds % 3600) / 60);
					const time_end_seconds = time_end_in_seconds % 60;
	...
	const renderHours = (
		...
		const renderedOuput: React.ReactNode[] = [];
		hoursData.forEach(range => {
			const firstDayKey = range.day[0];
			const lastDayKey = range.day[range.day.length - 1];
			const firstDay = daysOfWeek.find(d => d.key === firstDayKey);
			const lastDay = daysOfWeek.find(d => d.key === lastDayKey);
